...
gapp.register("kiri.platform", [], (root, exports) => {
    ...
    /**
     * Returns a slightly lighter or highlighted version of the given hex color,
     * suitable for indicating selection.
     * @param {string} hex - The original color in hex format (e.g. "#ff0000" or "ff0000").
    ...
    function highlightColor(hex) {
        ...
        // If color is light, darken; if dark, lighten
        if (luminance > 180) {
            // Darken by 20%
    ...
    function platformUpdateSelected() {
        ...
        if (extruders) {
            ...
            selection.for_widgets(w => {
                w.setColor(materialDisplay.dataset.color ? highlightColor(materialDisplay.dataset.color) : COLOR.selected);
            ...
            selection.for_widgets(w => {
                w.setColor(highlightColor(materialDisplay.dataset.color) || COLOR.selected);
            }, true);
        }
    }
    ...
    function platformSelect(widget, shift, recurse = true) {
        ...
        if (selection.contains(widget)) {
            ...
            widget.setColor(highlightColor(materialDisplay.dataset.color) || COLOR.selected);
    ...
    function platformDeselect(widget, recurse = true) {
        ...
        widget.setColor(materialDisplay.dataset.color || COLOR.deselected);
        platform.update_selected();
        selection.update_info();
        space.update();
    }
